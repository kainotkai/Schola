...
class PPOSettings():
    ...
    n_steps: int = 2048 #: Number of steps to run for each environment per update. This is the number of timesteps collected before updating the policy.
    batch_size: int = 64 #: Minibatch size for each update. This is the number of timesteps used in each batch for training the policy. Must be a divisor of `n_steps`.
    n_epochs: int = 10 #: Number of epochs to update the policy. This is the number of times the model will iterate over the collected data during training. More epochs can lead to better convergence but also overfitting.
    gamma: float = 0.99 #: Discount factor for future rewards. This determines how much the agent values future rewards compared to immediate rewards. A value of 0.99 means that future rewards are discounted by 1% per time step.
    gae_lambda: float = 0.95 #: Lambda parameter for Generalized Advantage Estimation (GAE). This parameter helps to balance bias and variance in the advantage estimation. A value of 1.0 corresponds to standard advantage estimation, while lower values will reduce variance but may introduce bias.
    clip_range: float = 0.2 #: Clipping range for the policy update. This is the maximum amount by which the new policy can differ from the old policy during training. This helps to prevent large updates that can destabilize training.
    normalize_advantage: bool = True #: Whether to normalize the advantages. Normalizing the advantages can help to stabilize training by ensuring that they have a mean of 0 and a standard deviation of 1. This can lead to more consistent updates to the policy.
    ent_coef: float = 0.0 #: Coefficient for the entropy term in the loss function. This encourages exploration by adding a penalty for certainty in the policy's action distribution. A higher value will encourage more exploration, while a lower value will make the policy more deterministic. Set to 0.0 to disable entropy regularization.
    vf_coef: float = 0.5 #: Coefficient for the value function loss in the overall loss function. This determines how much weight is given to the value function loss compared to the policy loss. A higher value will put more emphasis on accurately estimating the value function, while a lower value will prioritize the policy update.
    max_grad_norm: float = 0.5 #: Maximum gradient norm for clipping. This is used to prevent exploding gradients by scaling down the gradients if their norm exceeds this value. This can help to stabilize training, especially in environments with high variance in the rewards or gradients.
    use_sde: bool = False #: Whether to use State Dependent Exploration (SDE). This can help to improve exploration by adapting the exploration noise based on the current state of the environment. When set to True, it will use SDE for exploration instead of the standard exploration strategy.
...
class SB3ScriptArgs(ScriptArgs):
    ...
    # General Arguments
    timesteps: int = 3000 #: Total number of timesteps to train the agent. This is the total number of environment steps that will be used for training. This should be set based on the complexity of the environment and the desired training duration. A higher value will typically lead to better performance but will also increase training time.

    #   Misc Arguments
    pbar: bool = False #: Whether to display a progress bar during training. Requires TQDM and Rich to be installed.
    disable_eval: bool = False #: Whether to disable running evaluation after training. When set to True, it will skip evaluation after training completes.

    # Logging Arguments
    enable_tensorboard: bool = False #: Whether to enable TensorBoard logging.
    log_dir: str = "./logs" #: Directory to save TensorBoard logs.
    log_freq: int = 10 #: Frequency of logging training metrics to TensorBoard. This determines how often (in terms of training steps) the training metrics will be logged to TensorBoard. A value of 10 means that every 10 training steps, the metrics will be recorded. 
    callback_verbosity: int = 0 #: Verbosity level for callbacks. This controls the level of detail in the output from any callbacks used during training. 
    schola_verbosity: int = 0 #: Verbosity level for Schola-specific logging. This controls the level of detail in the output from Schola-related components during training. 
    sb3_verbosity: int = 1 #: Verbosity level for Stable Baselines3 logging. This controls the level of detail in the output from Stable Baselines3 components during training. 

    # Checkpoint Arguments
    save_replay_buffer: bool = False #: Whether to save the replay buffer when saving a checkpoint. This allows for resuming training from the same state of the replay buffer.
    save_vecnormalize: bool = False #: Whether to save the vector normalization statistics when saving a checkpoint. This is useful for environments where observations need to be normalized, and it allows for consistent normalization when resuming training.

    # Resume Arguments
    resume_from: str = None #: Path to a saved model to resume training from. This allows for continuing training from a previously saved checkpoint. The path should point to a valid model file created by Stable Baselines3. If set to None, training will start from scratch.
    load_vecnormalize: str = None #: Path to a saved vector normalization statistics file to load when resuming training. This allows for loading the normalization statistics from a previous training session, ensuring that the observations are normalized consistently when resuming training. If set to None, it will not load any vector normalization statistics.
    load_replay_buffer: str = None #: Path to a saved replay buffer to load when resuming training. This allows for loading a previously saved replay buffer, which can be useful for continuing training with the same set of experiences. The path should point to a valid replay buffer file created by Stable Baselines3. If set to None, it will not load any replay buffer, and a new one will be created instead.
    reset_timestep: bool = False #: Whether to reset the internal timestep counter when resuming training from a saved model. When set to True, it will reset the timestep counter to 0.

    # Network Architecture Arguments
    policy_parameters: List[int] = None #: A list of layer widths representing the policy network architecture. This defines the number of neurons in each hidden layer of the policy network. For example, [64, 64] would create a policy network with two hidden layers, each containing 64 neurons. If set to None, it will use the default architecture defined by the algorithm.
    critic_parameters: List[int] = None #: A list of layer widths representing the critic (value function) network architecture. This defines the number of neurons in each hidden layer of the critic network. For example, [64, 64] would create a critic network with two hidden layers, each containing 64 neurons. This is only applicable for algorithms that use a critic (e.g., SAC). If set to None, it will use the default architecture defined by the algorithm.
    activation: ActivationFunctionEnum = ActivationFunctionEnum.ReLU #: Activation function to use in the policy and critic networks. This determines the non-linear activation function applied to each layer of the neural networks. Common options include ReLU, Tanh, and Sigmoid. The choice of activation function can affect the performance of the model and may depend on the specific characteristics of the environment. Default is ReLU, but you can choose others based on your needs.

    # Training Algorithm Arguments
    algorithm_settings: Union[PPOSettings, SACSettings] = field(default_factory=PPOSettings) #: The settings for the training algorithm to use. This can be either `PPOSettings` or `SACSettings`, depending on the chosen algorithm. This property allows for easy switching between different algorithms (e.g., PPO or SAC) by simply changing the instance of the settings class. The default is `PPOSettings`, which is suitable for most environments unless specified otherwise.
    
    # List of plugins
    plugins: List[Sb3LauncherExtension] = field(default_factory=lambda: []) #: A list of Plugins that can be used to extend the behaviour of launch.py
