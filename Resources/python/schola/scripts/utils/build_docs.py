...
Usage examples:
  - Full run (doxygen -> breathe -> sphinx html):
      schola build-docs --plugin-folder .
  - Only Sphinx (skip doxygen/breathe):
    schola build-docs --no-doxygen --no-breathe-apidoc
  - Clean build with extra Sphinx options:
      schola build-docs --clean --sphinx-opts="-W -q"
  - Drop Doxygen XML and regenerated Breathe / autosummary API RST, then build:
      schola build-docs --rebuild-autodoc
...
def _remove_tree_if_exists(path: Path, description: str) -> None:
    ...
    if not path.exists():
        logger.debug("%s path does not exist, skipping: %s", description, path)
        return
    if not path.is_dir():
        logger.info("Removing %s file %s", description, path)
        path.unlink()
        return
...
def _prune_anonymous_doxygen_namespace_rst(breathe_api_out: Path) -> None:
    """
    Remove breathe-apidoc RST files for Doxygen anonymous namespaces.

    Doxygen names anonymous namespaces like ``@0``; ``.. doxygennamespace:: @0`` is not
    valid C++ for Sphinx/Breathe and triggers parser errors and internal assertions.
    """
    ns_dir = breathe_api_out / "namespace"
    if not ns_dir.is_dir():
        return
    for path in ns_dir.glob("*.rst"):
        try:
            text = path.read_text(encoding="utf-8")
        except OSError as e:
            logger.warning("Could not read %s: %s", path, e)
            continue
        if ".. doxygennamespace:: @" in text:
            logger.info(
                "Removing %s (anonymous Doxygen namespace; not supported by Breathe)",
                path,
            )
            try:
                path.unlink()
...
def main(
    ...
    rebuild_autodoc : bool
        If True, delete Doxygen XML output and autogenerated RST under
        ``API/Python`` (autosummary) and ``API/Unreal`` (breathe-apidoc) before
    ...
    python_api_out = sphinx_root / "API" / "Python"
    ...
    logger.debug("Python autosummary API output: %s", python_api_out)
    ...
    if rebuild_autodoc:
        logger.info(
            "Rebuilding autodoc artifacts (Doxygen xml, API/Python, API/Unreal)"
        )
        _remove_tree_if_exists(doxygen_xml, "Doxygen XML")
        _remove_tree_if_exists(python_api_out, "autosummary Python API")
        _remove_tree_if_exists(breathe_api_out, "Breathe Unreal API")
    ...
    # Anonymous Doxygen namespaces (e.g. @0) break Breathe; strip them even if apidoc was skipped.
    _prune_anonymous_doxygen_namespace_rst(breathe_api_out)
    ...
    sphinx_exit = build_main(sphinx_args)
    if sphinx_exit != 0:
        logger.error("Sphinx exited with code %s", sphinx_exit)
