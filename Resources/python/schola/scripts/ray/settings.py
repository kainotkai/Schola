...
class PPOSettings(RLLibAlgorithmSpecificSettings):
    ...
    """
    gae_lambda: float = 0.95 #: The lambda parameter for Generalized Advantage Estimation (GAE). This controls the trade-off between bias and variance in the advantage estimation.
    clip_param: float = 0.2 #: The clip parameter for the PPO algorithm. This is the epsilon value used in the clipped surrogate objective function. It helps to limit the policy update step size to prevent large changes that could lead to performance collapse.
    use_gae: bool = True #: Whether to use Generalized Advantage Estimation (GAE) for advantage calculation. GAE is a method to reduce the variance of the advantage estimates while keeping bias low. If set to False, the standard advantage calculation will be used instead.
    ...
    def get_parser(cls):
        ...
        parser.add_argument("--disable-gae", dest="use_gae", action="store_false", help="Disable the Generalized Advantage Estimation (GAE) for the PPO algorithm")
        parser.add_argument("--gae-lambda", type=float, default=0.95, help="The GAE lambda value for the PPO algorithm")
        parser.add_argument("--clip-param", type=float, default=0.2, help="The clip range for the PPO algorithm")
...
class IMPALASettings(RLLibAlgorithmSpecificSettings):
    ...
    def get_parser(cls): 
        parser = argparse.ArgumentParser(add_help=False)
        parser.add_argument("--disable-vtrace", dest="vtrace",action="store_false",help="Disable the V-trace algorithm")
        parser.add_argument("--vtrace-clip-rho-threshold", type=float, default=1.0, help="The clip threshold for V-trace rho values")
        parser.add_argument("--vtrace-clip-pg-rho-threshold", type=float, default=1.0, help="The clip threshold for V-trace rho values in the policy gradient")
...
class APPOSettings(IMPALASettings, PPOSettings):
    ...
    @classmethod
    def get_parser(cls):
        parser = argparse.ArgumentParser(parents=[IMPALASettings.get_parser(), PPOSettings.get_parser()],add_help=False)
...
class TrainingSettings():
    ...
    """
    timesteps: int = 3000 #: The number of timesteps to train for. This is the total number of timesteps to run during training. 
    learning_rate: float = 0.0003 #: The learning rate for  any chosen algorithm. This controls how much to adjust the model weights in response to the estimated error each time the model weights are updated. A smaller value means slower learning, while a larger value means faster learning.
    minibatch_size: int = 128 #: The size of the minibatch for training. This is the number of samples used in each iteration of training to update the model weights. A larger batch size can lead to more stable estimates of the gradient, but requires more memory and can slow down training if too large.
    train_batch_size_per_learner: int = 256 #: The number of samples given to each learner during training. Must be divisble by minibatch_size. 
    num_sgd_iter: int = 5 #: The number of stochastic gradient descent (SGD) iterations for each batch. This is the number of times to update the model weights using the samples in the minibatch. More iterations can lead to better convergence, but also increases the training time.
    gamma: float = 0.99 #: The discount factor for the reinforcement learning algorithm. This is used to calculate the present value of future rewards. A value of 0.99 means that future rewards are discounted by 1% for each time step into the future. This helps to balance the importance of immediate versus future rewards in the training process. A value closer to 1.0 will prioritize future rewards more heavily, while a value closer to 0 will prioritize immediate rewards.
    ...
    def populate_arg_group(cls, args_group):
        args_group.add_argument("-t", "--timesteps", type=int, default=3000, help="Number of timesteps to train for")
        args_group.add_argument("--learning-rate", type=float, default=0.0003, help="Learning rate for the PPO algorithm")
        args_group.add_argument("--minibatch-size", type=int, default=128, help="The size of the minibatch for training. Taken from the train batch given to each learner")
        args_group.add_argument("--train-batch-size-per-learner", type=int, default=256, help="Size of the minibatch given to each learner")
        args_group.add_argument("--num-sgd-iter", type=int, default=5, help="The number of SGD iterations for each batch")
        args_group.add_argument("--gamma", type=float, default=0.99, help="The discount factor for the PPO algorithm")
...
class ResourceSettings():
    ...
    """
    num_gpus: Optional[int] = 0 #: The number of GPUs to use for the training process. This specifies how many GPUs are available for the RLlib training job. If set to 0, it will default to CPU training. This can be used to leverage GPU acceleration for faster training times if available.
    num_cpus: Optional[int] = 1 #: The total number of CPUs to use for the training process. This specifies how many CPU cores are available for the RLlib training job. This can be used to parallelize the training process across multiple CPU cores, which can help to speed up training times. 
    ...
    @classmethod
    def populate_arg_group(cls, args_group):
        args_group.add_argument("--num-gpus", type=int, default=0, help="Number of GPUs to use")
        args_group.add_argument("--num-cpus", type=int, default=1, help="Number of CPUs to use")
        args_group.add_argument("--num-cpus-per-learner", type=int, default=1, help="Number of CPUs to use per learner process")
        args_group.add_argument("--num-gpus-per-learner", type=int, default=0, help="Number of GPUs to use per learner process")
        args_group.add_argument("--num-learners", type=int, default=0, help="Number of learner processes to use")
        args_group.add_argument("--num-cpus-for-main-process", type=int, default=1, help="Number of CPUs to use for the main process")
        args_group.add_argument("--using-cluster", action="store_true", help="Whether Ray is running on a cluster")
...
class LoggingSettings(): 
    ...
    """
    schola_verbosity: int = 0 #: Verbosity level for the Schola environment. This controls the level of detail in the logs generated by the Schola environment. A higher value will produce more detailed logs, which can be useful for debugging and understanding the training process. Default is 0 (no additional logging).
    rllib_verbosity: int = 1 #: Verbosity level for RLlib. This controls the level of detail in the logs generated by RLlib. A higher value will produce more detailed logs, which can be useful for debugging and understanding the training process. Default is 1 (standard logging).
    ...
    @classmethod
    def populate_arg_group(cls, args_group):
        args_group.add_argument("-scholav", "--schola-verbosity", type=int, default=0, help="Verbosity level for the Schola environment")
        args_group.add_argument("-rllibv", "--rllib-verbosity", type=int, default=1, help="Verbosity level for RLlib")
...
class ResumeSettings():
    ...
    """
    resume_from: Optional[str] = None #: Path to a checkpoint to resume training from. This allows you to continue training from a previously saved model checkpoint instead of starting from scratch. This is useful for long training jobs or if you want to experiment with different hyperparameters without losing progress. If set to None, training will start from scratch.
    ...
    @classmethod
    def populate_arg_group(cls,args_group):
        args_group.add_argument("--resume-from", type=str, default=None, help="Path to checkpoint to resume from")
...
class NetworkArchitectureSettings():
    ...
    """
    fcnet_hiddens: List[int] = field(default_factory=lambda: [512, 512]) #: The hidden layer architecture for the fully connected network. This specifies the number of neurons in each hidden layer of the neural network used for the policy and value function approximation. The default is [512, 512], which means two hidden layers with 512 neurons each. This can be adjusted based on the complexity of the problem and the size of the input state space.
    activation: ActivationFunctionEnum = ActivationFunctionEnum.ReLU #: The activation function to use for the fully connected network. This specifies the non-linear activation function applied to each neuron in the hidden layers of the neural network. The default is ReLU (Rectified Linear Unit), which is a commonly used activation function in deep learning due to its simplicity and effectiveness. Other options may include Tanh, Sigmoid, etc. This can be adjusted based on the specific requirements of the problem and the architecture of the neural network.
    use_attention: bool = False #: Whether to use an attention mechanism in the model. This specifies whether to include an attention layer in the neural network architecture. Note, this attends does not attend over the inputs but rather the timestep dimension.
    attention_dim: int = 64 #: The dimension of the attention layer. This specifies the size of the output from the attention mechanism if `use_attention` is set to True. The attention dimension determines how many features will be used to represent the output of the attention layer. A larger value may allow for more complex representations but will also increase the computational cost. The default is 64, which is a common choice for many applications.
...
class RLlibScriptArgs(ScriptArgs):
    ...
    algorithm_settings: Union[PPOSettings, APPOSettings, IMPALASettings] = field(default_factory=PPOSettings)
    ...
    network_architecture_settings: NetworkArchitectureSettings = field(default_factory=NetworkArchitectureSettings)
