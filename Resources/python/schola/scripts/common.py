...
class ActivationFunctionEnum(str, Enum):
    ...
    ReLU : Tuple[str, torch.nn.ReLU] = ("relu", torch.nn.ReLU) #: Rectified Linear Unit activation function.
    Sigmoid : Tuple[str, torch.nn.Sigmoid] = ("sigmoid", torch.nn.Sigmoid) #: Sigmoid activation function.
    TanH : Tuple[str, torch.nn.Tanh] = ("tanh", torch.nn.Tanh) #: Hyperbolic Tangent activation function.
...
def add_unreal_process_args(parser : argparse.ArgumentParser) -> argparse._ArgumentGroup:
    ...
    unreal_process_group.add_argument("--executable-path",default=None, type=str, help="Path to the standalone executable, when launching a standalone Environment")
...
class ScriptArgs:
    ...
    port: Optional[int] = None #: Port to connect to the Unreal Engine process, if None an open port will be automatically selected when running standalone. Port is required if connecting to an existing Unreal Engine process.
    executable_path: Optional[str] = None #: Path to the standalone executable, when launching a standalone Environment
    ...
    map: Optional[str] = None #: Map to load when launching a standalone Unreal Engine process
    fps:Optional[int] = None #: Fixed FPS to use when running standalone, if None no fixed timestep is used
    ...
    def make_unreal_connection(self):
        ...
        if self.launch_unreal:
           if(self.executable_path is None):
                raise ValueError("Unreal Engine path must be provided when launching a standalone Unreal Engine process")
           return StandaloneUnrealConnection("localhost", self.executable_path, self.headless, port=self.port, map=self.map, set_fps=self.fps, disable_script=self.disable_script)
        else:
